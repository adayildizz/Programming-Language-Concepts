  ...
  (define value-of
    (lambda (exp env)
      (cases expression exp
        ...
        ; #####################################################
        (proc-nested-exp (var countt name body)
                         (let ((theCount (expval->num (value-of (var-exp countt) env))))
                           (proc-val (nested-procedure var theCount name body env))))

        (call-nested-exp (rator rand countt)
          (let ((theProc (expval->proc (value-of rator env)))
                (arg (value-of rand env))
                (theCount (expval->num (value-of countt env))))
            (apply-procedure
             (cases proc theProc
               (nested-procedure (bvar count name body env) (nested-procedure bvar theCount name body env))
               (else theProc)) arg
             )))

        (letrec-nested-exp (p-name b-var countt p-body letrec-body )
          (let ((theCount (expval->num (value-of (var-exp countt) env))))
          (value-of letrec-body
            (extend-env-rec-nested p-name b-var p-body env theCount))))
      ...
      (cases proc proc1
        ...
        (nested-procedure  (var count name body saved-env)
                           (begin
                             ...
                             (value-of body (extend-env 'count (num-val count) (extend-env var arg saved-env)))))
