(module translator (lib "eopl.ss" "eopl")
  ...
  (define translation-of
    (lambda (exp env)
      ;(display "exp") (display exp) (newline) (newline)
      (cases expression exp
        ...
        ; #####################################################
        (proc-exp (var body)
                  ;(display var) (newline) (display body)(newline)(newline)
                  (proc-nested-exp
                   var
                   'count
                   'anonym
        ...
        ; #####################################################
        (call-exp (rator rand)
                  ;(display rator) (newline) (display rand)(newline)(newline)
                  (let* ((theRator (translation-of rator env))
                         (theRand (translation-of rand env))
                         (theCount (cases expression theRator
                                      (var-exp (var) (diff-exp (var-exp 'count) (const-exp -1)))
                                      (else (const-exp 1))
                                      )))
                  
                      (call-nested-exp theRator theRand theCount))       
        ...
        (letrec-exp (p-name b-var p-body letrec-body)
                    ;(display p-name) (newline) (display letrec-body)(newline)(newline)
                    (letrec-nested-exp p-name
                                       b-var
                                       'count
